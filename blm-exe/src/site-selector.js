...
class SiteSelector {
	...
	checkSiteStatus(site, timeout = 2000) {
		// Guard: skip sites without valid ip
		if (!site.ip) {
		...
		// Use port 80 as default for reachability check when port is null
		const checkPort = site.port ? Number(site.port) : 80;
		...
		return new Promise(resolve => {
			...
			socket.connect(checkPort, site.ip);
		});
	}
